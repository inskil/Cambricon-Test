...
extern "C" {
#endif
void SquaredDiffKernel(half* input1, half* input2, half* output, int32_t len);
#ifdef __cplusplus
}
#endif
void SquaredDiffKernel(half* input1, half* input2, half* output, int32_t len);

int MLUSquareDiffOp(float* input1,float* input2, float*output, int dims_a) {
 ...
 // output[0] = 0;
  ...
  if ( CNRT_RET_SUCCESS != cnrtInvokeKernel_V2((void*)&SquaredDiffKernel, dim, params, c, queue)) {
